...
function Feed(){
  ...
  useEffect(()=>{
    (async () =>{
      ...
      const response = await fetchCrosswords(id);
      if(response)
        setCrosswords(response.crosswords);
    })()
  },[user]);
  return(
    <main>
          ...
          {crosswords.map((item, key) => (
                <div key={item.id || key} className="crosswordTable" style={{width: "30vw"}}>
                  ...
                  <div className="controls">
                      <NavLink to={`/profile/${item.user.id}`} style={{display: "flex", gap: ".4vw", alignItems: "center", textDecoration: "none", minWidth: 0}}>
                        <img style={{width: "2vw"}} src={avatarIcon} alt="account picture" />
                        <p style={{color: "black", textOverflow: "ellipsis", whiteSpace: "nowrap", overflow: "hidden", width: "100%", margin: 0, minWidth: "6vw"}}>
                          {item.user.userName}
                        </p>
                      </NavLink>
